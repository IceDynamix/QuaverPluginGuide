...
- [Getting started with making a Quaver Editor plugin](#getting-started-with-making-a-quaver-editor-plugin)
    ...
    - [Making a plugin](#making-a-plugin)
        ...
        - [Note about Slider/Drag Int/Float 4 types](#note-about-sliderdrag-intfloat-4-types)
        - [Styling](#styling)
...
nearly useless. You'll have to do some manual debugging, commenting out code
piece by piece and printing to the Quaver logs (`Quaver/Logs/runtime.log`) with
the `print()` function in Lua. Or you can write the content of a variable into
a `imgui.TextWrapped()` in the plugin. It's up to you.
...
Until the `imgui_style_var` enum is implemented, copy and paste this into your
code. When this gets implemented, feel free to delete the table.
...
then placing them with the according action function**.
...
`util.retrieveStateVariables()`. But if a module is just defining functions and
